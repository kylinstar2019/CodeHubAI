...
describe('settingsBackup', () => {
  ...
  it('exports settings as module snapshots', async () => {
    ...
    localStorage.setItem('CodeHubAI-sidebar-expanded', 'false')
    localStorage.setItem('CodeHubAI-right-panel-width', '512')
    localStorage.setItem('notifications-enabled', 'true')
    localStorage.setItem('CodeHubAI:toast-enabled', 'false')
    localStorage.setItem('srv:local:last-directory', '/workspace/project')
    localStorage.setItem('srv:local:CodeHubAI-auto-approve-enabled', 'true')
  ...
  it('restores settings from module snapshots', async () => {
    ...
    expect(localStorage.getItem('CodeHubAI-sidebar-expanded')).toBe('false')
    expect(localStorage.getItem('CodeHubAI-right-panel-width')).toBe('512')
    ...
    expect(localStorage.getItem('srv:local:CodeHubAI-auto-approve-enabled')).toBe('true')
    expect(localStorage.getItem('CodeHubAI:toast-enabled')).toBe('false')
    expect(sessionStorage.getItem('CodeHubAI-active-server')).toBe('local')
  })
})
